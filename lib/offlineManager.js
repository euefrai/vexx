...
export const offlineManager = {
  ...
  async syncQueue() {
    ...
    for (const item of queue) {
      try {
        ...
        if (error) {
          ...
          // Se for erro de rede/conexão, mantemos na fila para tentar novamente
          if (error.message?.includes("Failed to fetch") || error.status === 0 || error.message?.includes("network")) {
            remainingQueue.push(item)
            failedCount++
          } else {
            // Se for erro permanente de regra de negócio, descartamos para não travar a fila
    ...
    if (successCount > 0 && failedCount === 0) {
      ...
      // Disparar evento customizado para que as páginas recarreguem seus dados
      window.dispatchEvent(new CustomEvent("vexx_offline_sync_complete"))
    } else if (successCount > 0 && failedCount > 0) {
      toast.update(toastId, {
        render: `Sincronização parcial: ${successCount} salvas, ${failedCount} pendentes por oscilação de rede.`,
        type: "warning",
        isLoading: false,
        autoClose: 5000
      })
